...
public class SnowyGlowingOakForestBiome extends RagemodModElements.ModElement {
	public static Biome biome;

	public SnowyGlowingOakForestBiome(RagemodModElements instance) {
		super(instance, 256);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				mobSpawnInfo.withSpawner(EntityClassification.CREATURE, new MobSpawnInfo.Spawners(EntityType.ZOMBIE, 12, 1, 2));
				mobSpawnInfo.withSpawner(EntityClassification.CREATURE, new MobSpawnInfo.Spawners(EntityType.FOX, 4, 1, 2));
				biome = new Biome.Builder().precipitation(Biome.RainType.SNOW).category(Biome.Category.FOREST).depth(0.7f).scale(0.4f)
						.temperature(-0.1f).downfall(0.3f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("ragemod:snowy_glowing_oak_forest"));
			}
		}
	}
