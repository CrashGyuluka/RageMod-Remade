...
public class ToxicWasteBiome {
	private static final ConfiguredSurfaceBuilder<?> SURFACE_BUILDER = SurfaceBuilder.DEFAULT
			.configured(new SurfaceBuilderBaseConfiguration(RagemodModBlocks.TOXIC_TOPPED_DIRT.defaultBlockState(),
					RagemodModBlocks.TOXIC_TOPPED_STONE.defaultBlockState(), RagemodModBlocks.TOXIC_TOPPED_STONE.defaultBlockState()));

	public static Biome createBiome() {
		BiomeSpecialEffects effects = new BiomeSpecialEffects.Builder().fogColor(-14432742).waterColor(-12469233).waterFogColor(-8007144)
				.skyColor(-14432742).foliageColorOverride(-16487142).grassColorOverride(-15817447)
				.ambientParticle(new AmbientParticleSettings(RagemodModParticles.TOXIC_PARTICLE, 0.006f)).build();
		...
		biomeGenerationSettings.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, register("trees", Feature.TREE
				.configured((new TreeConfiguration.TreeConfigurationBuilder(new SimpleStateProvider(Blocks.OAK_LOG.defaultBlockState()),
						new StraightTrunkPlacer(7, 2, 0), new SimpleStateProvider(Blocks.OAK_LEAVES.defaultBlockState()),
		...
		return new Biome.BiomeBuilder().precipitation(Biome.Precipitation.RAIN).biomeCategory(Biome.BiomeCategory.NONE).depth(0.1f).scale(1.6f)
				.temperature(0.7f).downfall(0.4f).specialEffects(effects).mobSpawnSettings(mobSpawnInfo.build())
	...
	public static void init() {
		...
		CONFIGURED_FEATURES.forEach((resourceLocation, configuredFeature) -> Registry.register(BuiltinRegistries.CONFIGURED_FEATURE, resourceLocation,
				configuredFeature));
		BiomeDictionary.addTypes(ResourceKey.create(Registry.BIOME_REGISTRY, BuiltinRegistries.BIOME.getKey(RagemodModBiomes.TOXIC_WASTE)),
				BiomeDictionary.Type.HILLS, BiomeDictionary.Type.SPARSE, BiomeDictionary.Type.MODIFIED, BiomeDictionary.Type.OVERWORLD);
